...
async def pay_sub(message: types.Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_invoice(userid, title='Приобретение подписки на сервис "Кибер Папер"',
...
async def process_pay_sub(pre_checkout_query: types.PreCheckoutQuery, state: FSMContext, bot: Bot):
    ...
    if current_state is None:
        ...
        await bot.send_message(chat_id=config.info_channel,
