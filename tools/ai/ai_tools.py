...
class OpenAI:
    ...
    async def get_dialogs(self, user_id: int) -> list:
        user = await self.session.get(User, id=user_id)
        if user and user.history:
            return user.history
        else:
            await self.reset_history(user_id)
            return await self.get_dialogs(user_id)
    ...
    async def get_resp(self, query: str, chat_id: int) -> tuple[str, str]:
        ...
        response = await self._query_gpt(chat_id, query, dialogs)
        answer = ''
        ...
        if response.choices and len(response.choices) > 1 and self.n_choices > 1:
            for index, choice in enumerate(response.choices):
                ...
                if index == 0:
                    await self.add_to_history_db(chat_id, role="assistant", content=content)
                answer += f'{index + 1}\u20e3\n'
                answer += content
                answer += '\n\n'
        elif response.choices and len(response.choices) >= 0:
            answer = response.choices[0]['message']['content'].strip()
            await self.add_to_history_db(chat_id, role="assistant", content=answer)
        else:
            answer = response.choices[0]['message']['content'].strip()
            await self.add_to_history_db(chat_id, role="assistant", content=answer)
    ...
    async def _query_gpt(self, user_id, query, dialogs):
        while self.retries < self.max_retries:
            try:
                if not dialogs:
                    await self.reset_history(user_id)
                    dialogs = await self.get_dialogs(user_id)

                await self.add_to_history_db(user_id, role="user", content=query)
                ...
                if exceeded_max_tokens or exceeded_max_history_size:
                    ...
                    try:
                        ...
                        await self.reset_history(user_id)
                        await self.add_to_history_db(user_id, role="assistant", content=summary)
                        await self.add_to_history_db(user_id, role="user", content=query)
