...
class OpenAIListener:
    ...
    async def _query_gpt_listen(self, query):
        while self.retries < self.max_retries:
            try:
                return await openai.ChatCompletion.acreate(api_key=self.api_key, model=self.model, messages=[{"role": "system", "content": self.listen_content}, {"role": "user", "content": query}],
                                                           **args)
