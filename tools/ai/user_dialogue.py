...
args = {
    "temperature": 0, "max_tokens": 1024, "top_p": 1, "frequency_penalty": 0, "presence_penalty": 0.8, "stop": None
    }
    ...
    async def _query_gpt(self, user_id, query):
        while self.retries < self.max_retries:
            try:
                ...
                return await openai.ChatCompletion.acreate(api_key=self.api_key, model=self.model, messages=self.history.user_dialogs[user_id], **args)
